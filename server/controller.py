...
DB_PATH = "app.db"  # banco SQLite será salvo nesse arquivo
...
def init_db(schema_path="videos.sql"):
    """Cria a tabela no banco a partir do arquivo videos.sql"""
...
def insert_video(data: dict):
    """Insere um registro na tabela videos"""
    cols = ", ".join(data.keys())
...
def mark_video_deleted(video_id: str):
    """Marca vídeo como 'trash' (movido para lixeira)"""
    with closing(get_conn()) as conn, conn:
